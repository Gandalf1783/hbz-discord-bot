...
const downloadPage = async (config, channel, page = '') => {
	...
	const indexfile = JSON.parse(
		fs.readFileSync('videos/index.json', { encoding: 'utf-8' }),
	)
	const url = `https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${config.youtubeChannel}&maxResults=10&order=date&type=video&key=${config.youtubeAPIKey}&pageToken=${page}`
	...
	Object.keys(fetch.data.items).map((video) => {
		...
		if (videoInfo.live !== 'none') return // Checking if the video is NO LIVESTREAM (no upcoming one or currently streaming one)

		if (
			!fs.existsSync(
				`videos/${videoInfo.id}.mp3` ||
